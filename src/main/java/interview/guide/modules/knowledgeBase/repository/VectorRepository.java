...
public class VectorRepository {
    ...
    public int deleteByKnowledgeBaseId(Long knowledgeBaseId) {
        log.info("delete vector data init: kbId={}", knowledgeBaseId);
        ...
        try {
            ...
            if (deletedRows > 0) {
                log.info("delete successfully: kbId={}, 删除行数={}", knowledgeBaseId, deletedRows);
            } else {
                log.info("data not found, skip: kbId={}", knowledgeBaseId);
            }

            return deletedRows;

        } catch (Exception e) {
            log.error("delete vector data failed: kbId={}, error={}", knowledgeBaseId, e.getMessage());
            throw new RuntimeException("delete vector data failed", e);
        }
    }
